...
public class CardsController extends Controller {
	...
	public void drawUserCardsPanel(Utente user, String title, String collection, List<Carta> cardsName,
			List<Utente> utenti, Utente toVisit, boolean fromSearchPanel) {
		cardsPanel = new CardsPanel();
		cardsPanel.setBounds(0, 0, 800, 600);
		cardsPanel.setTitleText(title);
				...
				if (!e.getValueIsAdjusting()) {
					...
					if (toVisit == null) {
						if (fromSearchPanel)
							...
							cartaController.drawUserCartaPanel(user, title, collection, cardsName, card, null, null, false);
					}

					else
						cartaController.drawUserCartaPanel(user, title, collection, cardsName, card, utenti, toVisit, false);
				}
			}
		});
		...
		cardsPanel.addStatsListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				cardsPanel.setVisible(false);
				StatsController statsController = new StatsController(frame);
				if(toVisit == null)
					statsController.disegnaPannelloStatistiche(user, null, null, collection);
				else
					statsController.disegnaPannelloStatistiche(user, toVisit, utenti, collection);
			}
		});
	}
