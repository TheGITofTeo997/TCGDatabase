...
public class CartaController extends Controller {
	...
	public void drawUserCartaPanel(Utente user, String title, String collectionName, List<Carta> cardsName, Carta c, List<Utente> utenti,
			Utente toVisit, boolean fromSearchPanel) {
		cardPanel = new CartaPanel();
		cardPanel.setBounds(0, 0, 800, 600);
		LogWriter.write("Richiesta del nome del pokemon allo stage precedente");
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (toVisit == null) {
					if (fromSearchPanel)
						...
						cardsController.drawUserCardsPanel(user, title, collectionName, cardsName, null, null, false);
				} else
					cardsController.drawUserCardsPanel(user, title, collectionName, cardsName, utenti, toVisit, false);
			}
		});
